...
let record_of_recordVal = function 
  ...
  |  _ -> error "Expected a Dictionary!"

(** has_duplicates is a helper function used for Record, which checks if there are any duplciate fields. *)
let rec has_duplicates l =
  match l with [] -> false | h :: t -> List.mem h t || has_duplicates t
let rec string_of_expval = function
  | NumVal n -> "NumVal " ^ string_of_int n
  | BoolVal b -> "BoolVal " ^ string_of_bool b
  | PairVal (ev1,ev2) -> 
    "PairVal("^string_of_expval ev1 ^","^ string_of_expval ev2^")"
