...
let rec is_subclass : string -> string -> class_env -> exp_val ea_result =
  fun id1 id2 class_env ->
    ...
    | Some(super,_fields, _methods) -> 
      if (super = id2) then return (BoolVal true) 
      else (if super = "object" then return (BoolVal false) 
        else is_subclass super id2 class_env)
...
and   
  ...
  | IsInstanceOf (e , id ) -> 
    eval_expr e >>= fun t -> 
    obj_of_objectVal t >>= fun (c_name,_) ->
      begin
      match List.assoc_opt id !g_class_env with
      | None -> error ("is_subclass: class " ^ id ^ " not found ")
      | Some (_super,_fields,_methods) -> 
      if c_name = id then return (BoolVal true) else 
